    ...
    <SenderShell active="Transfers">
      ...
      {transfer ? (
        <>
          ...
          {!quotes.some((quote) => quote.status === "SENT") && quotes[0] ? <section className="active-quote"><div className="active-quote-heading"><StatusBadge {...transferStatus(transfer.status)} /><h2>Quote version {quotes[0].version}</h2></div><p>{quotes[0].status === "ACCEPTED" ? "You accepted this quote. Funding instructions will appear next." : `This quote is ${quotes[0].status.toLowerCase()}.`}</p></section> : null}
