...
function mappingText(mapping: Record<string, string[]>) { return Object.entries(mapping).map(([country, values]) => `${country}: ${values.join(", ")}`).join("; "); }
function parseMapping(value: string) { return Object.fromEntries(value.split(/[;\n]/).map((item) => item.trim()).filter(Boolean).map((item) => { const [country, values] = item.split(":", 2); return [country?.trim().toUpperCase(), codes(values || "")]; })); }
...
export function AdminConsole() {
  ...
  const [configDraft, setConfigDraft] = useState({ origins: "AE", destinations: "PH", currencies: "AED, PHP", sendCurrencyMap: "AE: AED", receiveCurrencyMap: "PH: PHP", payoutMethodMap: "PH: BANK_TRANSFER, CASH_PICKUP, MOBILE_MONEY", quoteSlaMinutes: "45", quoteDefaultExpiryMinutes: "30", evidenceMaxSizeBytes: "8388608", contentTypes: defaultContentTypes.join(", "), broadcastMessage: "", maintenanceMessage: "", senderTransferLimitMinor: "", reason: "" });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (current) setConfigDraft({
        ...
        sendCurrencyMap: mappingText(current.sendCurrenciesByOrigin), receiveCurrencyMap: mappingText(current.receiveCurrenciesByDestination), payoutMethodMap: mappingText(current.payoutMethodsByDestination),
        contentTypes: current.evidenceAllowedContentTypes.join(", "), broadcastMessage: current.broadcastMessage || "", maintenanceMessage: current.maintenanceMessage || "", senderTransferLimitMinor: Object.values(current.transferLimitsByCurrency || {})[0]?.maximumAmountMinor || "", reason: ""
      });
  ...
  async function saveConfiguration(event: FormEvent) {
    ...
    const payoutMethodsByDestination = parseMapping(configDraft.payoutMethodMap);
    await mutate("/admin/configuration", { method: "POST", body: JSON.stringify({
      ...
      supportedOriginCountries: codes(configDraft.origins), supportedDestinationCountries: destinations, supportedCurrencies: codes(configDraft.currencies),
      sendCurrenciesByOrigin: parseMapping(configDraft.sendCurrencyMap), receiveCurrenciesByDestination: parseMapping(configDraft.receiveCurrencyMap), payoutMethodsByDestination,
